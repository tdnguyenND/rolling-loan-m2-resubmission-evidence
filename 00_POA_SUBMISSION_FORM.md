...
([`06` §3.3](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/06_UAT_Reports_Four_Journeys_M2.md#33-coverage-stability-and-findings)), and they hit something the delivery team had not: the refinance **failed to submit on the first
...
| **Open** | 7 signed Open transactions on mainnet, each `valid_contract = true`; captured screen by screen in all four preprod walkthroughs, including the **multi-pool** case where one borrow opens two loans | **70 run, 15 fail** — the `[Create loan]` suite. Most failures are the suite declaring its own **Tier-2** limits: what a connected-but-unsigned harness cannot assert, not product defects ([C-6](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/08_CORRECTIONS.md)) | [`05` §1.1](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/05_ONCHAIN_TRANSACTIONS_AND_REPAY.md#11-the-transactions-that-opened-these-loans) · [`06` §4.2](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/06_UAT_Reports_Four_Journeys_M2.md#42-verification-qc) |
| **Repay** | 3 signed mainnet repayments from the borrower's own funds, plus the settlement leg inside each of the 5 refinances. Fluid's indexer reports all 7 of its loans `loan_repaid`, `remainingDebt: 0`. Captured screen by screen on preprod **three times** — quote → wallet dialog → settled transaction — each closing the loan that walkthrough had just opened | **50 run, 12 fail** — the `[Repay]` suite. Two failures are the same defect as OI-1, found independently ([C-6](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/08_CORRECTIONS.md)) | [`05` §5](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/05_ONCHAIN_TRANSACTIONS_AND_REPAY.md#5-evidence-for-repayment-three-direct-repayments-and-repayment-within-a-refinance) |
| **Refinance** | 5 signed mainnet refinances; **25 transaction QC checks** (TC-01 … TC-25), **24 of which hold**; 5 further refinances captured screen by screen on preprod | **All 7 in-scope tests pass** on mainnet with the feature flag on — the CTA renders, its label carries a figure in both directions, it sits last in the footer, and it opens the preview. **The previous submission's “8/8” remains withdrawn** — [C-6](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/08_CORRECTIONS.md) | [`07` §B.2](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/07_ACCEPTANCE_CRITERIA_M2.md#b2-ac4-what-is-automated-and-what-is-not) |


Two notes on that last row. The negative control that a Dano loan offers no refinance also passes,
but it is counted under **View**, not among the seven. And the “8 / 8” is withdrawn because **two of
those eight had never reached their own assertion** — [C-6](https://github.com/tdnguyenND/rolling-loan-m2-resubmission-evidence/blob/main/08_CORRECTIONS.md)
sets out why. Every automated figure in this table is **no-sign**: real wallet reads,
`signTx`/`submitTx` stubbed.
...
- **C-6 — "the 8 automated refinance tests pass 8 / 8" is withdrawn.** We re-ran them on
  ...
  behind a flag that is off there; with the flag on, and after a harness fix, all 7 in scope. Worse, all eight carry a `KNOWN-FAIL (finding, app-vs-spec)` annotation
  ...
  the flag on: **87 of 87 Loan Details tests in scope have passed — across three runs, not one**;

...
| **8 automated UI tests** for the refinance surface (FN‑I7, FN‑I9 … FN‑I14, FN‑J10), Tier‑2 with a connected wallet — **no-sign**: the wallet's balance/UTxO reads are real, but `signTx` and `submitTx` are stubbed, so nothing is signed or broadcast ([`07` §C](./07_ACCEPTANCE_CRITERIA_M2.md#c-what-the-automated-suite-does-and-does-not-prove)) | **4 of 8 pass** on preprod (FN‑I7, FN‑I12, FN‑I14, FN‑J10); **0 of 8 on mainnet**, where the feature ships behind a flag that is off. The previous submission's **8 / 8 is withdrawn** | [`08` C‑6](./08_CORRECTIONS.md) |
| **The rest of the Loan Details suite** — 98 tests, Tier‑2, against the live **mainnet** app | **78 pass**, covering the **View** journey: overview, collateral list, health factor, APR, utilisation, money and rate formats. The 20 that do not pass are itemised in [`08` C‑6](./08_CORRECTIONS.md) | [`07` §B.2](./07_ACCEPTANCE_CRITERIA_M2.md#b2-ac4-what-is-automated-and-what-is-not) |
...
**The automated suite reaches two journeys of the four.** **View** is covered — 78 tests pass.
**Refinance** is covered in part — 4 of 8. **Open** and **repay** have suites in the harness that
have not been run, so they are evidenced by execution and manual QC, not by tests.

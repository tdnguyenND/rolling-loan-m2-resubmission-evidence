...
| **8 automated UI tests** for the refinance surface (FN‑I7, FN‑I9 … FN‑I14, FN‑J10), Tier‑2 with a connected wallet — **no-sign**: the wallet's balance/UTxO reads are real, but `signTx` and `submitTx` are stubbed, so nothing is signed or broadcast ([`07` §C](./07_ACCEPTANCE_CRITERIA_M2.md#c-what-the-automated-suite-does-and-does-not-prove)) | **5 of 8 pass** on mainnet with the feature flag on (FN‑I7, FN‑I9, FN‑I12, FN‑I14, FN‑J10); **0 of 8** with it off, which is how the deployment ships. The previous submission's **8 / 8 is withdrawn** | [`08` C‑6](./08_CORRECTIONS.md) |
| **The Loan Details suite** — Tier‑2, against the live **mainnet** app with the flag on. 98 tests run, **92 in scope** (the 2 Surf tests are a different protocol and 4 are display-level — [`08` C‑6](./08_CORRECTIONS.md) says which and why) | **83 of 92 pass.** The nine that do not are **six on one harness stub defect and three click-timeout flakes** — **no product defect**. Unscoped, the figure is 83 of 98 | [`08` C‑6](./08_CORRECTIONS.md) |
...
**The automated suite reaches all four journeys, and not one of them cleanly** — 83 of the 92 Loan
Details tests in scope, open 70 run with 15 failing, repay 50 run with 12 failing. No failure inside
the reported scope is a product defect.

...
| **8 automated UI tests** for the refinance surface (FN‑I7, FN‑I9 … FN‑I14, FN‑J10), Tier‑2 with a connected wallet, on live mainnet data — **no-sign**: the wallet's balance/UTxO reads are real, but `signTx` and `submitTx` are stubbed, so nothing is signed or broadcast ([`07` §C](./07_ACCEPTANCE_CRITERIA_M2.md#c-what-the-automated-suite-does-and-does-not-prove)) | **8 / 8 pass** — of the surface and its arithmetic, **not** of settlement | [`01` §3](./01_Integration_Test_Report_M2.md) |
